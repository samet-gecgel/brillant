...
export default class GroupSymbol implements IGroupSymbol {
    ...
    getTick() {
        if (!this.Symbol) {
            throw new Error("group symbol does not have any symbol");
        }
        const { ...tick } = this.Symbol.Tick.toJSON();
        if (this.SpreadDiff !== 0) {
            const diff = ~~(this.SpreadDiff / 2);
            tick.Ask += diff * Math.pow(10, -this.Symbol.Digits)
            tick.Bid -= (this.SpreadDiff - diff) * Math.pow(10, -this.Symbol.Digits)
        }
        return {
            Ask: tick.Ask,
            Bid: tick.Bid,
